...
import { SystemPermissionRoleTemplateIdActionResourceCompoundUniqueInputObjectSchema } from './SystemPermissionRoleTemplateIdActionResourceCompoundUniqueInput.schema'

const makeSchema = () => z.object({
  id: z.string().optional(),
  roleTemplateId_action_resource: z.lazy(() => SystemPermissionRoleTemplateIdActionResourceCompoundUniqueInputObjectSchema).optional()
}).strict().superRefine((obj, ctx) => {
        const presentTop = (k: string) => (obj as any)[k] != null;
        const singles: string[] = ["id"] as string[];
        const groups: string[][] = [["roleTemplateId","action","resource"]] as string[][];
